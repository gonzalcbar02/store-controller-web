  ...
  useEffect(() => {
    const loadProductData = async () => {
      ...
      try {
        ...
        setValue("description", producto.descripcion)
      } catch (error) {
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      const payload = {
        ...
        id_armario: productData.id_armario,
      }
      ...
      // Redirigir después de 2 segundos al armario correspondiente
      setTimeout(() => {
        navigate(`/armarios/${productData.id_armario}`)
      }, 2000)
    } catch (error: any) {
      console.error("Error al actualizar el producto:", error)
      setSubmitMessage({
        type: "error",
        text: error.message || "Error al actualizar el producto. Por favor, intenta de nuevo.",
      })
    } finally {
      setIsLoading(false)
    }
    ...
    <>
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        <div>
          ...
          <p className="text-2xl font-light text-gray-500 mt-5">Modifica los datos de "{productData.name}"</p>
        </div>

        <nav className="my-5">
          <Link
            className="bg-[#075b8f] hover:bg-[#012e4a] text-white font-bold py-3 px-10 text-lg cursor-pointer transition-colors rounded"
            to={`/armarios/${productData.id_armario}`}
          >
            Volver al Armario
          </Link>
        </nav>
      ...
      <form className="mt-10 bg-white shadow-lg rounded-lg p-10" onSubmit={handleSubmit(onSubmit)}>
        ...
        {userEmail && (
          <div className="mb-6 p-4 rounded-md bg-blue-50 border border-blue-200 text-blue-800">
            Editando producto del usuario: {userEmail}
          </div>
        )}
          ...
          <div className="mt-8">
            ...
            <div className="max-w-sm rounded-lg overflow-hidden shadow-lg border border-gray-200">
              ...
              <div className="px-4 py-3">
                <div className="font-bold text-lg mb-1">{watch("name") || productData.name}</div>
                <p className="text-gray-500 text-sm">{watch("description") || productData.descripcion}</p>
              </div>
            </div>
          </div>
